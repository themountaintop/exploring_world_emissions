...
Our analysis shows that, while global emissions are going up, some countries are reducing their emissions, but global temperatures continue to rise.
...
- Data Cleaning and Database Creation
    - The datasets we used included one showing amounts of emissions by each country per year, which included totals, per capita and breakdowns by subcategories such as coal or flaring, a second one showing the population of each country per year, and a third showing the average temperature change of each country per year.
...
- Tableau Visualizations
    - A tableau dashboard was created to show what the temperature and emissions trends have been over the last 20-60 years. The dynamic chart "Total Emissions By Country" shows which countries are the top emitters by year selection. Since 2006 the order of top emitters are China, USA, Russia, Japan, and Germany. 
    - The heat map "Emissions Per Capita" shows which countries have the highest emissions based on population by year selection. This visualization shows us that while China is a higher emitter, the amount per capita is lower than that of the USA and Russia. 
    - The multi-line graph "Global Emissions by Year" shows trending emissions over time. Most countries trend upward in emissions, but some have reduced their emissions. Globally, all total emissions continue to increase over time.
...
- Machine Learning Exploration
    - The Total Emissions and Temperature Change vs Year plots were created, as shown above in segment 2. This was sort of the beginning of where we realized there may be issues modeling the data. First off, the temperature changes bounce up and down quite a bit, which would make it difficult for a model to make accurate predictions. Additionally, as the emissions went up for China, so did the temperature change. Germany however reduced their emissions, but their temperatures still went up on average. When looking at the correlations of all the features to the temperature change, we found that besides the year column all the other correlations were fairly weak across the entire dataset. However, when just looking at the global data, almost all the correlations were strong. This told us there must be conflicting trends in the data.

- Machine Learning Processing
    - Processing for our machine learning models started by removing countries with too many missing values and filling in other missing data with zero. We found that the data was not substantially altered whether we removed the null values or filled them in. After the cleaning process, we created custom encoding for our country values by sorting them by total emissions. We did this because other encoding methods such as OneHotEncoding created too many columns compared to the amount of data we had for each country. After encoding our country values, we dropped the country and country code columns. Then we split our data into a training set using the years 1961 through 2011, and a test set using 2012 to 2021. Finally, we scaled the data to standardize it.
- For tuning the Support Vector and Random Forest Regressors a RandomSearchCV was used. This performed cross validation on the training set while randomly searching through hyperparameter values which were provided in ranges or lists. The hyperparameter values that performed the best were selected and used to make predictions.

##### Results of the ML Models

- ARIMA Modeling
    - The ARIMA (Autoregressive Integration Moving Average) model has the potential to predict future values based on the data it's fed. However, during our analysis it did not perform up to the standard we needed in order for it to make any accurate predictions. When plotting our diagnostics from the model, the Histogram and Normal Q-Q plots showed good indications that the residuals were normally distributed. The Normal Q-Q plot follows a linear trend with the samples taken, however our ACF (Autocorrelation Plot) shows low to high variance and the values do not decay towards zero which suggest the model is not quite capturing the intended information from the data it was given and is/was not going to accurately predict any future values.
...
- SQL foreign keys
- Creating a working ARIMA model
- Tableau attributes and dimensions
- Conflicting trends hurt ML Models

### Future Plans

- If we were to continue on with a future analysis we would try to find more data by month or even by day, instead of just year. We would also create more Tableau visualizations, like temperature change by country. Finally, we would refine the data processing and model tuning to see if we could increase performance and get the ARIMA model to work better.
- What we would do differently is try to focus on global data only, since the conflicting emission trends of the countries made it hard for the models to learn. Additionally, we’d look for more global data that might have an impact on temperature changes.
